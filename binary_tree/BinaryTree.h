...
// 求以r为根的孩子兄弟法表示的树的高度
	int HeightChildSiblingHelper(const BinTreeNode<ElemType> *r) const;
	void DegreeHelper(BinTreeNode<ElemType>* node, int& maxDegree);
...
template <class ElemType>
int BinaryTree<ElemType>::HeightChildSiblingHelper(const BinTreeNode<ElemType> *r) const	{
	if(r == NULL) return 0;
	else{
		int heightChild, heightSibling;
		heightChild = HeightChildSiblingHelper(r->leftChild);
		heightSibling = HeightChildSiblingHelper(r->rightChild);
...
template <class ElemType>
int BinaryTree<ElemType>::HeightChildSibling() const {
	// 求孩子兄弟表示法的树的高度
	return HeightChildSiblingHelper(root);
}


template <class ElemType>
int BinaryTree<ElemType>::Degree(){ 
	int maxDegree = 0;
	DegreeHelper(root, maxDegree);
    return maxDegree;
}

template <class ElemType>
void BinaryTree<ElemType>::DegreeHelper(BinTreeNode<ElemType>* node, int& maxDegree){
	if(node == NULL) return;
	int degree = 0;
	BinTreeNode<ElemType>* p = node->leftChild;
	while(p != NULL){
		degree++;
		p = p->rightChild;
	}
	if(degree > maxDegree) maxDegree = degree;
	DegreeHelper(node->leftChild, maxDegree);
	DegreeHelper(node->rightChild, maxDegree);
}
